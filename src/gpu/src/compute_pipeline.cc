...
class ComputePipelineImpl : public Object {
 public:
  void __init__(VkPipelineLayout pipeline_layout, const uint32_t* shader, size_t size) {
  ...
  void __del__() { vkDestroyPipeline(device_, pipeline_, NULL); }
...
ComputePipeline ComputePipeline::Create(VkPipelineLayout pipeline_layout, const uint32_t* shader, size_t size) {
  return Make<ComputePipelineImpl>(pipeline_layout, shader, size);
}

ComputePipeline::operator VkPipeline() const noexcept { return *impl_; }
