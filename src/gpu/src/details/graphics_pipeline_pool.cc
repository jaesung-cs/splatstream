...
class GraphicsPipelinePoolImpl : public EnableHandleFromThis<GraphicsPipelinePool, GraphicsPipelinePoolImpl> {
 public:
  ...
  GraphicsPipeline Allocate(const GraphicsPipelineCreateInfo& create_info) {
    ...
    if (it != pipelines_.end()) {
      return GraphicsPipeline::Create(HandleFromThis(), create_info, it->second);
    }
    ...
    return GraphicsPipeline::Create(HandleFromThis(), create_info, pipeline);
  }

  void Free(const GraphicsPipelineCreateInfo& create_info, VkPipeline pipeline) { pipelines_[create_info] = pipeline; }
