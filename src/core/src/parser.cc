...
class VKGS_CORE_API ParserImpl {
 public:
  ...
  GaussianSplats CreateGaussianSplats(size_t size, const float* means_ptr, const float* quats_ptr,
                                      const float* scales_ptr, const float* opacities_ptr, const uint16_t* colors_ptr,
                                      int sh_degree, int opacity_degree) {
    ...
    int opacity_size = 0;
    int opacity_packed_size = 0;
    switch (opacity_degree) {
      case -1:
      case 0:
        opacity_size = 1;
        opacity_packed_size = 1;
        break;
      case 1:
        opacity_size = 4;
        opacity_packed_size = 1;
      ...
      case 2:
        opacity_size = 9;
        opacity_packed_size = 2;
        break;
      case 3:
        opacity_size = 16;
        opacity_packed_size = 4;
        break;
      default:
    ...
    auto opacity = gpu::Buffer::Create(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                                       size * opacity_size * sizeof(float));

    auto position_opacity = gpu::Buffer::Create(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                                                size * 4 * sizeof(float));
    auto cov3d = gpu::Buffer::Create(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, size * 8 * sizeof(float));
    auto sh = gpu::Buffer::Create(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, size * sh_packed_size * 4 * sizeof(uint16_t));
    auto opacity_sh =
        gpu::Buffer::Create(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, size * opacity_packed_size * 4 * sizeof(uint16_t));
  ...
  GaussianSplats LoadFromPly(const std::string& path, int sh_degree = -1) {
    ...
    auto position_opacity = gpu::Buffer::Create(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, point_count * 4 * sizeof(float));
    auto cov3d = gpu::Buffer::Create(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, point_count * 8 * sizeof(float));
    auto sh =
...
Parser Parser::Create() { return Make<ParserImpl>(); }

GaussianSplats Parser::CreateGaussianSplats(size_t size, const float* means, const float* quats, const float* scales,
...
GaussianSplats Parser::LoadFromPly(const std::string& path, int sh_degree) const {
